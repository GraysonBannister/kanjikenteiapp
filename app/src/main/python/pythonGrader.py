...
# --                                                            ; {{{1
#
# File        : kanjidraw/pythonGrader.py
...
def grade_user_kanji(user_strokes, target_kanji_unicode, data=None, fuzzy=False):
    ...
    :param user_strokes: List of strokes drawn by the user.
    :param target_kanji_unicode: The Unicode of the target kanji.
    :param data: Kanji data loaded from data.json.
    ...
    target_kanji_char = chr(int(target_kanji_unicode, 16))

    if len(user_strokes) not in data or target_kanji_char not in data[len(user_strokes)]:
        return 0  # No match if the target kanji is not in the data for the given stroke count.

    target_strokes = data[len(user_strokes)][target_kanji_char]

    # Convert user strokes to Kanji object
    user_kanji = Kanji(user_strokes)
    target_kanji = Kanji(target_strokes)

    # Use strict or fuzzy matching based on the parameter
    if fuzzy:
        return fuzzy_match(user_kanji, target_kanji)
    else:
        return strict_match(user_kanji, target_kanji)

    # The return score line seems unnecessary here, as it's not defined in this scope.
    # If you have additional logic that sets a 'score' variable, make sure it's included before this line.
    # return score
